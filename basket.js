...
const basketCounterEl = document.querySelector('.cart-link span');
const basketTotalEl = document.querySelector('.basketTotal');
const basketTotalValueEl = document.querySelector('.basketTotalValue');
const basketEl = document.querySelector('.basket');

document.querySelector('.cart-link').addEventListener('click', () => {
  basketEl.classList.toggle('hidden_sc');
});

const basket = {};

document.querySelector('.card-boxes').addEventListener('click', event => {
...
function renderNewProductInBasket(productId) {
  const productRow = `
    <div class="basketRow" data-id="${productId}">
      ...
      <div>
        <span class="productCount">${basket[productId].count}</span> piece
      </div>
